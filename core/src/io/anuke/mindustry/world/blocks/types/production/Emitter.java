...
public abstract class Emitter extends PowerBlock{
	...
	public void getStats(Array<String> list){
		...
		if(hasLasers){
			...
			list.add("[powerinfo]Max power transfer/second: " + getPowerTransferPerSecond());
		}
	...
	public String getPowerTransferPerSecond() {
		return Strings.toFixed(powerSpeed * 60, 2);
	}
	
	@Override
	public CharSequence getKeyStat() {
		return Bundles.get("text.blocks.transfersec")+": " + getPowerTransferPerSecond();
	}
	...
	protected boolean isInterfering(Tile target, int rotation){
		if(target.block() instanceof Emitter ){
			Emitter other = (Emitter) target.block();
